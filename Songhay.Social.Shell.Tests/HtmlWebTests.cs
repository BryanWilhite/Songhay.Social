...
public class HtmlWebTests
{
    ...
    public void ShouldLoad(string location)
    {
        ...
        _testOutputHelper.WriteLine($"{nameof(htmlDoc.DocumentNode)}: {htmlDoc.DocumentNode.InnerHtml}");

        var metaTwitterImage = htmlDoc.DocumentNode.SelectSingleNode("//meta[@name='twitter:image:src']");
        _testOutputHelper.WriteLine($"{nameof(metaTwitterImage)}: {metaTwitterImage?.OuterHtml ?? "[null]"}");

        var metaTwitterHandle = htmlDoc.DocumentNode.SelectSingleNode("//meta[@name='twitter:site']");
        _testOutputHelper.WriteLine($"{nameof(metaTwitterHandle)}: {metaTwitterHandle?.OuterHtml ?? "[null]"}");

        var metaTwitterTitle = htmlDoc.DocumentNode.SelectSingleNode("//meta[@name='twitter:title']");
        _testOutputHelper.WriteLine($"{nameof(metaTwitterTitle)}: {metaTwitterTitle?.OuterHtml ?? "[null]"}");

        var metaTwitterDescription = htmlDoc.DocumentNode.SelectSingleNode("//meta[@name='twitter:description']");
        _testOutputHelper.WriteLine($"{nameof(metaTwitterDescription)}: {metaTwitterDescription?.OuterHtml ?? "[null]"}");

        var metaOgImage = htmlDoc.DocumentNode.SelectSingleNode("//meta[@property='og:image']");
        _testOutputHelper.WriteLine($"{nameof(metaOgImage)}: {metaOgImage?.OuterHtml ?? "[null]"}");

        var metaOgTitle = htmlDoc.DocumentNode.SelectSingleNode("//meta[@property='og:title']");
        _testOutputHelper.WriteLine($"{nameof(metaOgTitle)}: {metaOgTitle?.OuterHtml ?? "[null]"}");

        var metaOgDescription = htmlDoc.DocumentNode.SelectSingleNode("//meta[@property='og:description']");
        _testOutputHelper.WriteLine($"{nameof(metaOgDescription)}: {metaOgDescription?.OuterHtml ?? "[null]"}");

        var title = htmlDoc.DocumentNode.SelectSingleNode("//head/title");
        _testOutputHelper.WriteLine($"{nameof(title)}: {title?.OuterHtml ?? "[null]"}");
    }
